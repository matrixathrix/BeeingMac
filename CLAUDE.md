...
| `NowTab.kt` | The rating flow. Header row = large neutral "Beeing" title (`headlineLarge`, `onSurface`) on the left, then a weight spacer pushes the enlarged `StatusStrip` pill (⬢ cell-hive · ring x/8 · 🍯 honey pots — 23sp emojis, 17sp numbers, 24dp ring; thin white outline; taps to Hive) hard against the ⋮ menu button at the right edge (just 2dp between them). Rating card: **fixed** "How was your hour?" title (no adaptive per-hour text), then the pending-hour chip(s) — two `PendingHourChip`s (each `weight(1f)`, both carry a countdown so they're equal height) when two hours compete, else one full-width chip; then comb→tags→notes→save. `TagPickerSection`'s collapsed "+N more"/"edit tags" `AssistChip` is neutral-toned. Caught-up hero with the "🔒 Lock phone and bee mindful🐝" button + `TodayStripCard`. Sends `ACTION_LOCK_PHONE` broadcast if `isLockAccessibilityServiceEnabled`, else shows a dialog that opens Settings → Accessibility |
...
| `CombStrip.kt` | The rating input: 10 soft rounded-vertex hexagons (friendly, not sharp), zoned band tints at rest, hybrid honey fill, drag loupe, haptics. The drag loupe reserves **no** vertical space — it hangs in a zero-height anchor and rises above the strip on drag, free to overlap the chips above (temporary). `scoreWord()`, `PointyHexShape` (now rounded — name kept) |
