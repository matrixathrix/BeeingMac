...
| `CombStrip.kt` | The rating input: 10 lightly-rounded-vertex hexagons (`PointyHexShape` vertex radius `0.12f` — eased corners, not sharp spikes, but crisper than before), zoned band tints at rest, hybrid honey fill, drag loupe, haptics. The drag loupe **tracks the finger** — it reads raw `dragX` from the pointer (clamped to the strip), not the selected cell's center, so it slides under the thumb. It reserves **no** vertical space — hangs in a zero-height anchor and rises above the strip on drag, free to overlap the chips above (temporary). `scoreWord()`, `PointyHexShape` (name kept) |
